...
class AuthorizationRepositoryPostgres(AuthorizationRepository):
    ...
    def usuario_puede_actualizar_probabilidad_cierre(self, rut_usuario: str, id_proceso_comercial: int) -> bool:
        with obtener_conexion() as conn:
            with conn.cursor() as cur:
    ...
    def usuario_puede_actualizar_poliza(self, rut_usuario: str, numero_poliza: str) -> bool:
        with obtener_conexion() as conn:
            with conn.cursor() as cur:
                ...
                for row in rows:
                    if row['codigo_permiso'] == 'ACTUALIZAR_POLIZA_TODOS':
                        return True
                    if row['codigo_permiso'] == 'ACTUALIZAR_POLIZA_PROPIOS':
                        tiene_permisos = True
                ...
                query = '''
                    select PRO.rut_ej_comercial_asignado
                    from Poliza P
                    inner join Cliente C
                    on P.id_cliente = C.id
